...
const AdminAddProduct = () => {
  ...
  const addProduct = async () => {
    ...
    setLoading(true); // Set loading state to true when adding a product
    await axios.post("/admin/products", newProduct);
    setNewProduct({ name: "", price: "", image_url: "" });
    const res = await axios.get("/admin/products");
    setProducts(res.data);
    setLoading(false); // Set loading state to false after the product is added
    toast.success("Product added");
  };

  const deleteProduct = async (id) => {
    setLoading(true); // Set loading state to true when deleting a product
    await axios.delete(`/admin/products/${id}`);
    const res = await axios.get("/admin/products");
    setProducts(res.data);
    setLoading(false); // Set loading state to false after the product is deleted
